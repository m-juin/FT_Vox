...
namespace Vox::Engine::Meshs
{
	...
	bool VoxelMesh::Draw()
	{
		...
		vkCmdDrawIndexed(buffer, this->_indexMemData.elemCount, 1, this->_indexMemData.memoryOffset, this->_vertexMemData.memoryOffset, 0);
		return true;
	}
	void VoxelMesh::SetMemoryDatas(VkDeviceSize vMemoryOffset, VkDeviceSize vMemoryReserved, VkDeviceSize iMemoryOffset,
								   VkDeviceSize iMemoryReserved)
	{
	...
	void VoxelMesh::WriteInBuffers(Engine::Rendering::Buffers::Buffer &vBuffer,
								   Engine::Rendering::Buffers::Buffer &iBuffer)
	{
		if (this->_vertexMemData.elemCount > 0)
			vBuffer.UpdateAtOffset(1, this->_vertexMemData.memoryOffset, this->_vertexMemData.datas.data(),
								   this->_vertexMemData.memorySize);
		if (this->_indexMemData.elemCount > 0)
			iBuffer.UpdateAtOffset(1, this->_indexMemData.memoryOffset, this->_indexMemData.datas.data(),
								   this->_indexMemData.memorySize);
	}
} // namespace Vox::Engine::Meshs